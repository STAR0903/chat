...
- 腾讯云 API 调用会产生费用，需确保账号余额充足。
- `data/history.txt` 会累计对话记录，若需重置可手动删除该文件。
- 角色设定可通过修改 `data/set.txt` 调整 AI 回复风格（如关怀话术、话题方向）。


## 改进方向（生产环境适配）      
实际上，这个项目是为了答辩演示而生的简单产物。还存在多客户端隔离、密钥安全管理、错误处理和资源释放等问题。

1. 为每个客户端连接创建独立临时目录，支持多用户并发。
2. 采用环境变量或加密配置文件管理密钥。
3. 完善错误处理逻辑，避免程序崩溃。
4. 增加音频格式校验与转码功能，兼容更多输入格式。
5. 实现对话历史清理机制，避免文件过大。